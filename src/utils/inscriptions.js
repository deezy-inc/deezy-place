...
const getInscriptionsForAddress = async (address) => {
  const response = await axios.get(
    `${TURBO_API}/wallet/${address}/inscriptions`
  );
...
export const isTextInscription = (utxo) =>
  /(text\/plain|application\/json)/.test(utxo?.content_type);

export const isImageInscription = (utxo) =>
  /(^image)(\/)[a-zA-Z0-9_]*/gm.test(utxo?.content_type);
