...
const OrdinalsArea = ({ className, space }) => {
    ...
    useEffect(() => {
        const loadUtxos = async () => {
            setUtxosReady(false);
            const { data: { inscriptionsWithUtxo }} = await axios.get(`/api/inscriptions/${nostrAddress}?offset=0&limit=5`);
            console.log('inscription', inscriptionsWithUtxo)

            setOwnedUtxos(inscriptionsWithUtxo);
            setFilteredOwnedUtxos(inscriptionsWithUtxo);
            setUtxosReady(true);
        }
        loadUtxos();
