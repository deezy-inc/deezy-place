...
function useConnectWallet() {
    ...
    const onConnectHandler = async ({ metamask, xverse }) => {
        ...
        SessionStorage.set(SessionsStorageKeys.ORDINALS_ADDRESS, walletOrdinalsAddress);
        SessionStorage.set(SessionsStorageKeys.PAYMENT_ADDRESS, walletPaymentAddress);
    ...
    useEffect(() => {
        ...
        const ordinalsAddress = SessionStorage.get(SessionsStorageKeys.ORDINALS_ADDRESS);
        const paymentAddress = SessionStorage.get(SessionsStorageKeys.PAYMENT_ADDRESS);
        if (pubKey) {
            setOrdinalsPublicKey(pubKey);
        }
        if (walletName) {
