...
import { getInscription, takeLatestInscription, getNostrInscription } from "@services/nosft";
        ...
        const fetchInscriptions = async () => {
            try {
                ...
                for (let i = 0; i < totalInscriptions; i += chunkSize) {
                    const inscriptionChunk = collection.inscriptions.slice(i, i + chunkSize);

                    const inscriptionPromises = inscriptionChunk.map(async (_inscription) => {
                        try {
                            ...
                            inscription.isOwner =
                                nostrAddress && inscription.owner && nostrAddress === inscription.owner;
                ...
                <div className="row g-5">
                    {utxosReady && openOrders.length > 0 && (
                        <>
                            ...
                            {utxosReady && openOrders.length <= collection.inscriptions.length - 1 && (
