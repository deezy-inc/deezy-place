...
import { getAddressInfo, getOrdinalsAddressInfo } from "@utils/address";
import { TAPROOT_MESSAGE } from "@utils/wallet";
import { DEFAULT_DERIV_PATH, NETWORK, BOOST_UTXO_VALUE, TESTNET } from "@lib/constants.config";
...
function getXverseInputParams({ utxo, inputAddressInfo }) {
    return {
        hash: utxo.txid,
        index: utxo.vout,
        witnessUtxo: {
            value: utxo.value,
            script: Buffer.from(inputAddressInfo.script, "hex"),
        },
        tapInternalKey: inputAddressInfo.pubkey,
        sequence: 0xfffffffd,
    };
...
function createXversePsbt({ utxo, inputAddressInfo, destinationBtcAddress, sendFeeRate, output }) {
    const psbt = new bitcoin.Psbt({ network: NETWORK });
    // Input
    const inputParams = getXverseInputParams({ utxo, inputAddressInfo });
...
export const signAndBroadcastUtxoByXverse = async ({ pubKey, address, utxo, destinationBtcAddress, sendFeeRate }) => {
    const inputAddressInfo = await getOrdinalsAddressInfo(pubKey);
    const basePsbt = createXversePsbt({ utxo, inputAddressInfo, destinationBtcAddress, sendFeeRate });
    let psbtBase64 = "";
...
export async function signAndBroadcastUtxo({ pubKey, utxo, address, destinationBtcAddress, sendFeeRate, walletName }) {
    if (walletName === "xverse") {
        return signAndBroadcastUtxoByXverse({ pubKey, address, utxo, destinationBtcAddress, sendFeeRate });
    }
...
export async function createPsbtForBoost({ pubKey, utxo, destinationBtcAddress }) {
...
export async function signPsbtForBoost({ psbt, utxo, pubKey }) {
    const inputAddressInfo = getAddressInfo(pubKey);
