...
export async function getMetamaskSigner(metamaskDomain) {
  ...
  const toSign = `0x${Buffer.from(TAPROOT_MESSAGE(metamaskDomain)).toString(
    "hex"
  )}`;
...
export async function signAndBroadcastUtxo({
  pubKey,
  utxo,
  destinationBtcAddress,
  sendFeeRate,
...
export async function createPsbtForBoost({
  pubKey,
  utxo,
  destinationBtcAddress,
}) {
  ...
  virtualToSign.data.inputs.forEach((input, i) => {
    if (!input.finalScriptWitness) {
      const tx = bitcoin.Transaction.fromBuffer(
        virtualToSign.data.inputs[i].nonWitnessUtxo
      );
