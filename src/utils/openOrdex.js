...
async function inscriptionPage() {
    ...
    if (
        Number(inscriptionIdentifier).toString() ==
        inscriptionIdentifier.toString()
    ) {
    ...
    const processSellerPsbt = async (_sellerSignedPsbt) => {
        ...
        if (sellerSignedPsbtBase64) {
            ...
            document.getElementById("btnBuyInscriptionNow").style.display =
                "revert";
            document.getElementById(
    ...
    submitSignedSalePsbt = async () => {
        ...
        setTimeout(async () => {
            const signedContent =
                document.getElementById("signedSalePsbt").value;
            let signedSalePsbt;
            if (
                signedContent.startsWith("02000000") ||
                signedContent.startsWith("01000000")
            ) {
                const sellerSignedTx =
                    bitcoin.Transaction.fromHex(signedContent);
...
Missing:     ${satToBtc(-changeValue)} BTC`;
    ...
    displayBuyPsbt = async (psbt, payerAddress, title, successMessage) => {
        ...
        const payerCurrentMempoolTxIds = await getAddressMempoolTxIds(
            payerAddress
        );
...
See transaction details on <a href="${baseMempoolUrl}/tx/${txId}" target="_blank">block explorer</a>.`;
    ...
    if (!sellerSignedPsbt) {
        sellerSignedPsbt = await getLowestPriceSellPSBGForUtxo(
            inscription.output
        );
    }
    if (sellerSignedPsbt) {
